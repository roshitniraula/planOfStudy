...
export function GradTermChart({ data }: GradChartProps) {
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
      <h3 className="font-serif font-semibold text-mnsu-maroon mb-4">Students by Expected Graduation</h3>
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 4, right: 16, left: 0, bottom: 24 }}>
          ...
          <Tooltip />
          <Bar dataKey="count" name="Students" fill="#782F40" radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
export function CompetencyChart({ data }: CompetencyChartProps) {
  const colors = ["#782F40", "#C8A84B", "#5a7f9e"];
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
      <h3 className="font-serif font-semibold text-mnsu-maroon mb-4">Approved Experiences by Competency</h3>
      <ResponsiveContainer width="100%" height={220}>
...
export function StageDistChart({ data }: StageDistProps) {
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5 col-span-2">
      <h3 className="font-serif font-semibold text-mnsu-maroon mb-4">Progress Stage Distribution (Highlighted Experiences)</h3>
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 4, right: 16, left: 0, bottom: 8 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
          ...
          <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
          <Tooltip />
          <Bar dataKey="count" name="Experiences" fill="#C8A84B" radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
